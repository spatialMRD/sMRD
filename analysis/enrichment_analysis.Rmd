...
genes_to_keep <- gene_counts %>% filter(n >= 2) %>% pull(gene)
...
for (patient in responders) {
  ...
  post_samples <- patient_snv %>% filter(timepoint == 1) %>% pull(sample_id)
  # just get the first sample_id where timepoint == -1
  pre_sample <- patient_snv %>% filter(timepoint == -1) %>% pull(sample_id) %>% first()
...
plot_volcano <- function(data, title) {
  ...
  p <- ggplot(data, aes(x = meanzscore, y = -log10(pval_corrected))) +
    ...
    geom_point(data = subset(data, !significant),
...
sig_responders <- results_responders %>% filter(pval_corrected < 0.00001) %>% filter(meanzscore > 0)
sig_non_responders <- results_non_responders %>% filter(pval_corrected < 0.00001) %>% filter(meanzscore > 0)
...
non_responder_snv <- df_snv %>% filter(patient_id %in% non_responders) %>% filter(gene %in% g)
```
