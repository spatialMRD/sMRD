...
def main():
    ...
    for patient_id in patient_ids:
        ...
        mrd_detected = mrd_detected.loc[
            mrd_detected.detected == 1, :
        ].sample_id.tolist()
        ...
        df["euclidean_dist"] = np.sqrt(
            (df["x"] - closest_coords[0]) ** 2
            + (df["y"] - closest_coords[1]) ** 2
            + (df["z"] - closest_coords[2]) ** 2
        )

        # get the correlation between euclidean_dist and cnv_dist
        corr = float(df[["euclidean_dist", "cnv_dist"]].corr().iloc[0, 1])
        corrs.append({"patient_id": patient_id, "corr": corr})
